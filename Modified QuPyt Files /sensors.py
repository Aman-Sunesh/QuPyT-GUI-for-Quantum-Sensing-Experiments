...
class DAQ(Sensor):
    ...
    def __init__(self, configuration: Dict[str, Any]) -> None:
        ...
        self.daq_apd_input: str = "Dev1/ai1"
    ...
    def acquire_data(self, synchroniser: Optional[Synchroniser] = None) -> np.ndarray:
        ...
        print(f"DAQ_VOLTAGE: {mean_v:.6f}")

        total_counts = int(np.sum(samples))
    ...
    def close(self) -> None:
        ...
        try:
            if getattr(self, 'daq_task', None) is not None:
                self.daq_task.close()
                logging.info("DAQ closed".ljust(65, ".") + "[done]")
            else:
                logging.debug("DAQ.close() called, but no task to close.")
        except Exception as e:
            logging.warning(f"Error closing DAQ task: {e}")
