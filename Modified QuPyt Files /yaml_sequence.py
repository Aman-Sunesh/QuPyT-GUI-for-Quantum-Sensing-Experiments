...
class YamlSequence:
    ...
    def write(self, output_path: Union[str, Path] = None) -> None:
        """
        Dump the pulse_sequence to YAML.  If output_path is given,
        write there; otherwise fall back to ~/.qupyt/.../sequence.yaml.
        """
        # 1) pick the right file
        if output_path is None:
            seq_path = set_up.get_seq_dir() / "sequence.yaml"
        else:
            seq_path = Path(output_path)

        # 2) inject sequencing metadata
        ...
        with open(seq_path, "w", encoding="utf-8") as f:
            yaml.dump(self.pulse_sequence, f, sort_keys=False)
